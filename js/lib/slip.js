...
var slipjs = {
	_refreshCommon: function(wide_high, parent_wide_high) { // 作用：当尺寸改变时，需要重新取得相关的值
		...
		that.parent_wide_high      = parent_wide_high      || that._parent_node[that.offset];
	...
	_initCommon: function(core,param) { // 作用：初始化
		...
		that._parent_node = that.core.parentNode;
		if(that.direction == 'x'){
		...
		that.parent_wide_high   = param.parent_wide_high || that._parent_node[that.offset];
...
var SlipPage = _fun.clone(slipjs);
	SlipPage._init = function(core,param) { // 作用：初始化
		var that           = this;
		that.num           = param.num;
		if(that.num != 1){ // 当只有一张图片的时候取消所有特效，不做任何操作
			that._initCommon(core,param);
			that.page          = 0;
			that.loop      = param.loop;
			...
			if(that.loop){
				var _single       = that.core.querySelectorAll('a');
				var _single_first = _single[0].cloneNode(true);
				var _single_last  = _single[that.num - 1].cloneNode(true);
				that.core.insertBefore(_single_last, _single[0]);
				that.core.appendChild(_single_first);
				//that.core.style.width = that.wide_high / that.num * (that.num + 2) + "px";
				that._initial_coordinates = -that.parent_wide_high;
			}else{
				that._initial_coordinates = 0;
			}
			param.no_follow ? (that._move = that._moveNoMove, that.next_time = 500) : that.next_time = 300;
			that._pos(that._initial_coordinates);
			that._parent_node.style['webkitTransform'] = 'translate3d(0px, 0px, 0px)';
		}
	};
	...
	SlipPage._move = function(evt) { // 触摸中,跟随移动
		...
		if(!that._movestart){
			...
			if(!that.loop){
				...
				that.offset_x = that._dis_x + that.xy;
			}
	...
	SlipPage.backward = function(){
		var that = this;
		that.change_time && that._stop();
		that._backward();
	};
	SlipPage.forward = function(){
		var that = this;
		that.change_time && that._stop();
		that._forward();
	};
	SlipPage._backward = function(e){
		...
		if(cur < 0){
			if(!that.loop){
				cur = 0;
				that.firstPageFun && that.firstPageFun(e);
			}else{
				cur = -1;
			}
		}
		that.toPage(cur, that.next_time);
	};
	SlipPage._forward = function(e){
		var that = this;
		var cur = that.page += 1;
		if(cur > that.num - 1){
			if(!that.loop){
				cur = that.num - 1;
				that.lastPageFun && that.lastPageFun(e);
			}else{
				cur = that.num;
			}
		}
	...
	SlipPage._transitionEnd = function(e) { // 动画结束
		...
		if(that.loop){ // 无限轮换模式下启用
			if(that.page >= that.num){
				that.toPage(0, 0);
			}else if(that.page <= -1){
				that.toPage(that.num - 1, 0);
			}
		}
	...
	SlipPage._autoChange = function() { // 作用：自动轮换
		...
		that._autoChangeSet = setInterval(function() {
			that._page_medium = that.page + 1;
        	that.page != that.num - 1 ? that.page += 1 : that.page = 0;
			if(that.loop){
				that.toPage(that._page_medium, that.next_time);
			}else{
	...
	SlipPage.refresh = function(wide_high, parent_wide_high) { // 可在外部调用，作用：当尺寸改变时（如手机横竖屏时），需要重新取得相关的值。这时候就可以调用该函数
		...
		if(this.loop){
			this._initial_coordinates = -this.parent_wide_high;
		}
	...
	SlipPx._insertSlipBar = function(param) { // 插入滚动条
		...
		that._bar_show_ing = true; // 滚动条的显示状态
		if(that.direction == 'x'){
			var _bar_css = 'height: 5px; position: absolute; top:1px;z-index: 10; pointer-events: none;';
			var _bar_shell_css      = 'opacity: '+that._bar_shell_opacity+'; left:2px; bottom: 2px; right: 2px; height: 6px; position: absolute; z-index: 10; pointer-events: none;';
		}else{
			var _bar_css = 'width: 5px; position: absolute; left:1px; z-index: 10; pointer-events: none;';
			var _bar_shell_css      = 'opacity: '+that._bar_shell_opacity+'; top:2px; bottom: 2px; right: 2px; width: 6px; position: absolute; z-index: 10; pointer-events: none; ';
		}
		var _default_bar_css = ' background-color: rgba(0, 0, 0, 0.5); border-radius: 11px; -webkit-transition: cubic-bezier(0.33, 0.66, 0.66, 1);' ;
		...
		that._parent_node.appendChild(that._bar_shell);
	...
	SlipPx.pos = function (x, time) {// 滑动到指定位置
		var that = this;
		if(that.xy != x){
			var t = time || 0;
			that._posTime(x, t);
			if(!that.perfect && t == 0){
				that._showBar();
				setTimeout(function(){that._hideBar();}, 100);
			}
			that.no_bar || that._slipBarTime(t);
		}
	};
	SlipPx._startAni = function () {// 作用：完美模式下的改变，目标对象位置
		var that = this,
			startX = that.xy,
			startTime = Date.now(),
			step, easeOut,
	...
	SlipPx._hideBar = function() {// 作用：隐藏滚动条
		...
		that._bar.style.webkitTransitionDuration = '0ms';
		that._bar_show_ing = false;
	};
